...
class WebStaticAssetTests(WebTestCase):
    ...
    def test_settings_dialog_contains_device_and_speed_controls(self):
        ...
        try:
            ...
            self.assertIn('id="voice-input-test-clip-control" class="voice-input-tuning-group" data-requires-backend', page)
            self.assertIn("<legend>Recognition Test Clip</legend>", page)
            self.assertIn('id="voice-input-test-clip-input" class="input-text"', page)
            self.assertIn('id="test-voice-input-clip-btn" class="my-button" type="button" data-requires-backend', page)
            self.assertIn("Runs the current saved recognition settings on a chosen clip", page)
    ...
    def test_frontend_css_contains_responsive_layout_guards(self):
        ...
        try:
            ...
            self.assertIn('grid-template-areas: "title status toggle"', css)
            self.assertIn('#toggle-sidebar-btn { grid-area: toggle; justify-self: end;', css)
            ...
            self.assertIn('grid-template-areas: "title toggle" "status status"', css)
    ...
    def test_frontend_js_wires_flexible_voice_input(self):
        ...
        self.assertIn("async function testVoiceInputClip", script)
        self.assertIn("function requestVoiceTranscription", script)
        self.assertIn("function ensureVoiceInputReadyForTestClip", script)
        ...
        self.assertIn("testVoiceInputClipBtn?.addEventListener('click', testVoiceInputClip)", script)
        self.assertIn("saveVoiceInputSettings({autoLoadHandsFree: false})", script)
        self.assertIn("Transcribing test clip", script)
        self.assertIn("Test clip transcript ready to review", script)
        self.assertIn("No speech detected in the test clip", script)
