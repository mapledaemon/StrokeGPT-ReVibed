...
class WebStaticAssetTests(WebTestCase):
    ...
    def test_frontend_css_contains_responsive_layout_guards(self):
        ...
        try:
            ...
            self.assertIn("--chat-max-width: 61.25rem", css)
            self.assertIn("--chat-feed-gap: 0.875rem", css)
            self.assertIn("--chat-avatar-size: 2rem", css)
            self.assertIn("--composer-min-height: 3rem", css)
            ...
            self.assertIn("#chat-view { flex: 1 1 auto; min-height: 0; padding: var(--shell-gutter);", css)
            ...
            self.assertIn("#status-text { width: min(var(--chat-max-width), 100%); min-height: 1.35em;", css)
