...
class WebStaticAssetTests(WebTestCase):
    ...
    def test_frontend_css_contains_responsive_layout_guards(self):
        ...
        try:
            ...
            self.assertIn("#chat-messages-container { display: flex; flex-direction: column; width: min(var(--chat-max-width), 100%);", css)
            self.assertIn(".chat-message-container { display: flex; align-items: flex-end; gap: var(--space-3);", css)
            self.assertIn("max-width: min(100%, var(--chat-message-max-width))", css)
            ...
            self.assertIn("#message-input-line { width: min(var(--chat-max-width), 100%); margin-inline: auto;", css)
