...
class WebStaticAssetTests(WebTestCase):
    ...
    def test_frontend_css_contains_responsive_layout_guards(self):
        ...
        try:
            ...
            self.assertIn("#message-input-line { grid-column: 1 / -1; grid-template-columns: 1fr; align-items: stretch; }", css)
            self.assertIn("#status-text { width: 100%; min-width: 0; min-height: var(--composer-min-height); margin: 0;", css)
            self.assertIn("#chat-compose-row { grid-template-columns: var(--voice-control-width) minmax(0, 1fr); }", css)
            self.assertIn("#voice-control-buttons { grid-column: 1; width: var(--voice-control-width); }", css)
            self.assertIn("#status-text { grid-column: 2; min-width: 0; justify-content: flex-start; text-align: left; }", css)
