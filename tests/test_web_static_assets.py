...
class WebStaticAssetTests(WebTestCase):
    ...
    def test_frontend_css_contains_responsive_layout_guards(self):
        ...
        try:
            ...
            self.assertIn("#message-input-line { width: min(var(--chat-max-width), 100%); min-height: var(--composer-min-height); margin-inline: auto;", css)
            self.assertIn("grid-template-columns: minmax(0, 1fr) auto", css)
            self.assertIn("#message-input-line { grid-template-columns: 1fr; align-items: stretch; }", css)
            self.assertIn("#status-text { width: min(var(--chat-max-width), 100%); min-height: 1.25em;", css)
