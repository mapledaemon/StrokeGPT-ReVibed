...
class InstallScriptTests(unittest.TestCase):
    def test_windows_installer_prompts_for_optional_runtime_dependencies(self):
        ...
        self.assertIn('winget install --id $PythonWingetId -e --source winget | Out-Host', script)
        self.assertIn('$pythonInstallExitCode = $LASTEXITCODE', script)
        ...
        self.assertIn("winget install --id Ollama.Ollama -e --source winget | Out-Host", script)
        self.assertIn('$ollamaInstallExitCode = $LASTEXITCODE', script)
